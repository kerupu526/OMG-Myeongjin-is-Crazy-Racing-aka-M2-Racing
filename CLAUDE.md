...
- 네트워킹: Netcode for GameObjects + Unity Relay + Unity Lobby (방 코드 생성/입장, 온라인 1v1)
  ...
  - 27~28차 작업(2026-07-12)에서 착수. Netcode for GameObjects는 Milestone 1(기본 연결 + 차량
    ...
    트랙/체크포인트/GameManager를 온라인 씬에 올려 바퀴/타이머/승패를 호스트 권위로 복제)를
    코드+자동검증까지 완료(라이브 2인 테스트는 사용자 확인 대기). 아이템 동기화(2b)/Relay/Lobby는
    아직 미착수. 자세한 내용은 아래 "진행 상황"의 27~29차 작업 항목 참고.
...
- **우선순위 4 (Netcode) — Milestone 1 완료(실제 2인 접속까지 확인) + Milestone 2a(레이스 상태
  서버-권위 동기화) 코드+자동검증 완료(라이브 2인 테스트는 사용자 확인 대기)**.
  다음 범위: Milestone 2b — 아이템 스폰/획득/사용(폭탄 등)의 서버-권위 동기화. Milestone 3 —
  Unity Relay(방 코드 발급/입장), Unity Lobby(대기방/방 목록). 전부 아직 미착수. 로컬 핫싯(2P
  스플릿)은 CLAUDE.md 기획 자체가 "온라인 1v1"만 명시하고 있어서 범위 아님.
...
- **29차 작업 완료(2026-07-12) — Netcode Milestone 2a: 레이스 상태 서버-권위 동기화(코드+자동검증)**
  ...
  - **아직 사용자 라이브 확인 필요(이 환경 한계)**: 실제 2인(ParrelSync 호스트+클론) 접속으로만
    검증 가능한 것들 — (a) 호스트에서 kinematic 상대 차의 체크포인트 트리거로 바퀴가 실제로
    카운트되는지, (b) `NetworkVariable` 복제 타이밍(카운트다운/타이머/결과가 양쪽에 제때 뜨는지),
    (c) 스타트 그리드 위치. **주의: 레이스는 2명이 다 접속해야 시작됨**(`requiredPlayers=2`) — 호스트
    혼자선 브리핑에서 대기. 사용자 라이브 테스트 체크리스트는 세션 마지막 메시지 참고.

## 다음 단계 후보 (30차 작업)
1. **Milestone 2a 라이브 확인 먼저**: 사용자가 ParrelSync 호스트+클론으로 `NetworkRace.unity`를
   실제로 접속해서 바퀴 카운트/카운트다운/타이머/승패 복제가 맞는지 확인해야 함(29차 참고, 이
   환경에서 자동 검증은 통과했지만 실제 2인 접속은 사용자만 가능). 여기서 나오는 피드백/버그
   수정이 2b보다 우선.
2. **Netcode Milestone 2b(아이템 서버-권위 동기화)**: 아이템 스폰/획득/사용(폭탄 등)을 서버
   권위로 동기화. 선행 작업 — 네트워크 차량 프리팹에 `ItemSlots` 추가(29차에서 `LapTracker`는
   이미 추가했지만 `ItemSlots`는 2b 범위로 유예함), `NetworkRace.unity`에 아이템 스포너 배치.
   현재 2a 씬은 아이템 스포너를 의도적으로 뺐음(`BuildTrackEnvironment`가 트랙/체크포인트만 만듦).
3. **또는 Netcode Milestone 3(Unity Relay/Lobby)**: 직접 IP 대신 방 코드 발급/입장 — Unity Cloud
   프로젝트는 이미 연동돼 있음(`cloudProjectId` 확인됨). 2b와 3 중 어느 쪽을 먼저 할지는 사용자 확인.
4. 25차에서 남겨둔 배경 소품/프리미티브 오브젝트(용암존 바위, 오아시스 갈대, 방송 카메라 삼각대,
   파이어볼 스파이크, 경고 표지판)의 실제 플레이 화면 육안 확인 — 아직 확인 안 됨.
3. 미확정 디자인 항목 정리: 아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP
   리스킨 여부, 아프리카TV/네더요새 게이지 밸런스 수치.
4. **참고**: 이번 세션에서 유니티 라이브 에디터의 콘솔 로그를 사용자가 매번 복사해줄 필요 없이
   `%LOCALAPPDATA%\Unity\Editor\Editor.log` 파일을 직접 읽어서 진단하는 방법을 알게 됨 — 다음에
   사용자가 에디터에서 뭔가 이상하다고 보고하면 이 로그부터 먼저 확인해볼 것.
