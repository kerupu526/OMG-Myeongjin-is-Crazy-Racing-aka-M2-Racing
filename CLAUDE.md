...
- 네트워킹: Netcode for GameObjects + Unity Relay + Unity Lobby (방 코드 생성/입장, 온라인 1v1)
  ...
  - 27~28차 작업(2026-07-12)에서 착수. Netcode for GameObjects는 Milestone 1(기본 연결 + 차량
    ...
    통과 확인됨(28차). Relay/Lobby는 아직 연결 안 됨. 자세한 내용은 아래
...
### 미착수
- **우선순위 4 (Netcode) — Milestone 1 완료(코드+자동검증+실제 2인 접속 테스트까지 전부 확인됨)**.
  다음 범위: 아이템 스폰/획득/사용(폭탄 등)의 서버-권위 동기화, 레이스 상태(타이머/승패)의
  서버-권위 동기화, Unity Relay(방 코드 발급/입장), Unity Lobby(대기방/방 목록) 전부 아직 미착수.
  로컬 핫싯(2P 스플릿)은 CLAUDE.md 기획 자체가 "온라인 1v1"만 명시하고 있어서 범위 아님.
...
- **28차 작업 완료(2026-07-12) — Netcode Milestone 1 실제 2인 접속 테스트, 발견된 버그 4개 수정**:
  ...
  내용은 위 27차 항목의 "최종 검증 완료" 참고. **다음 작업은 다른 에이전트/세션에 인계.**

## 다음 단계 후보 (29차 작업)
1. **Netcode 다음 마일스톤 선택**: Milestone 2(아이템 스폰/획득/사용의 서버-권위 동기화, 레이스
   상태(타이머/승패) 서버-권위 동기화) 또는 Milestone 3(Unity Relay로 직접 IP 대신 방 코드 발급/
   입장 — Unity Cloud 프로젝트는 이미 연동돼 있음, `cloudProjectId` 확인됨) 중 어느 쪽을 먼저
   할지 사용자와 확인 필요. 현재 네트워크 차량은 `LapTracker`/`ItemSlots`가 없는 최소 구성이라
   (27차 참고), Milestone 2를 하려면 먼저 이것들을 네트워크 차량 프리팹에 추가하는 작업부터
   필요함.
2. 25차에서 남겨둔 배경 소품/프리미티브 오브젝트(용암존 바위, 오아시스 갈대, 방송 카메라 삼각대,
