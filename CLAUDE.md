...
### 미착수
- **우선순위 3 (스테이지 게이지 시스템)**: `StageGaugeSystem` 추상 클래스, 비키니시티(산소)/
  아프리카TV(멘탈)/네더요새(체온) 게이지 전부 미구현. `M2.Stage` 네임스페이스 자체가 없음.
...
## 다음 단계 후보 (4차 작업)
CLAUDE.md 우선순위상 다음은 **우선순위 3: 스테이지 게이지 시스템**이 자연스러운 다음 단계.
1. `M2.Stage.StageGaugeSystem` 추상 클래스 설계 (최대치, 초당 변화량, 회복/고갈 이벤트 등 3개 스테이지가
   공유할 공통 인터페이스 — 상속으로 재사용, 중복 구현 금지 원칙 준수).
2. 1차로는 비키니시티 산소 게이지부터 구현 (다른 두 스테이지보다 우선순위 1과 가장 맞닿아 있고,
   `TestTrackBuilder`가 이미 만들어 둔 테스트 트랙 위에서 바로 검증 가능).
3. 게이지 고갈/화면 연출(붉어짐 등), "비법 놓친 횟수" 카운트, 완주 시 별점(★) 계산 로직 연동.
4. 진행하면서 미확정 항목(아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP 리스킨 여부)도
   같이 정리.
