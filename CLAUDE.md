...
- 네트워킹: Netcode for GameObjects + Unity Relay + Unity Lobby (방 코드 생성/입장, 온라인 1v1)
  ...
  - 27~28차 작업(2026-07-12)에서 착수. Netcode for GameObjects는 Milestone 1(기본 연결 + 차량
    ...
    스킴 + `NetworkItemSlots`/`NetworkItemSpawnManager` + 효과 RPC)를 완료 — **2a·2b 통합 라이브
    2인 테스트(ParrelSync)까지 통과 확인됨(30차)**. Relay/Lobby(Milestone 3)는 아직 미착수.
...
### 미착수
- **우선순위 4 (Netcode) — Milestone 1 + 2a(레이스 상태 서버-권위 동기화) + 2b(아이템 스폰/획득/
  사용 서버-권위 동기화) 전부 완료, 실제 2인 라이브 접속 테스트까지 통과 확인됨(30차, 2026-07-13)**.
...
- **30차 작업 완료(2026-07-13) — Netcode Milestone 2b: 아이템 스폰/획득/사용 서버-권위 동기화(코드+자동검증)**
  ...
  - **`M2Net:` 진단 로그**: 스폰 롤/픽업/재스폰/아이템 사용/폭발/기절/방패 막음/오너측 이펙트 적용에
    추가 — 라이브 테스트로 어디서 막히는지 추적용. **2a·2b 라이브 테스트가 통과했으므로 다음 세션에
    2a·2b 로그 전부 제거 정리 커밋할 것(31차 후보 1번).**
  - **통합 라이브 테스트 통과(2026-07-13, 사용자 확인) — 2a·2b 전부 실제 2인 접속에서 동작 확인**:
    사용자가 ParrelSync 호스트+클론으로 `NetworkRace.unity` 접속해서 확인. 접속 UI 숨김, 카운트다운,
    호스트의 양쪽 바퀴 카운트, 타이머/승패(레이스 종료까지 로그로 확인), 6지점 픽업 동기화, 호스트/
    클라 양쪽 픽업+재스폰, 슬롯 표시, 가속/방패/폭탄 사용, 호스트 권위 폭탄 판정까지 전부 정상.
    **Milestone 1·2a·2b 모두 실제 라이브 검증 완료.**
  - **라이브 테스트 후속 수정 2건(같은 세션)**:
    1. **폭탄 기절이 차를 실제로 안 멈추던 문제**: 기절 이펙트가 유일하게 코루틴(`ResolveBombServer`)
       에서 피해자(다른 차량 인스턴스)의 오너에게 RPC로 전달되는 경로였음. 4개 오너 이펙트 RPC
       (기절/부스트/방어막/방패소모)에 **오너 측 실행 로그**를 넣어 다음 테스트로 원인을 특정하려 했고,
       기절 지속시간을 명시적으로 전달(`NetworkItemSlots.bombStunDuration`, 1.0초 — 로컬 0.6초보다
       길게)하도록 함. 사용자 재테스트 결과 "잘 된다"로 해결 확인(계측+지속시간 조정으로 정상화).
    2. **같은 자리에 죽치고 앉아 원하는 아이템 나올 때까지 반복 뽑는 캠핑 방지**(사용자 지적):
       `NetworkItemSpawnManager`가 재스폰 시점에 스폰 지점 반경 `noRespawnPlayerRadius`(기본 3m) 안에
       플레이어가 있으면 재롤을 미루고, 자리를 뜬 뒤에야 새 아이템이 나오도록 함. 사용자 확인 완료.

## 다음 단계 후보 (31차 작업)
1. **`M2Net:` 진단 로그 제거 정리 커밋(최우선, 쉬움)**: 2a·2b 라이브 테스트가 전부 통과했으므로
   더 이상 필요 없음. `NetworkRaceBootstrap`(스폰), `NetworkItemSpawnManager`(스폰 롤/픽업/재스폰),
   `NetworkItemSlots`(아이템 사용/폭발/기절/방패/오너측 이펙트 적용)의 `Debug.Log("M2Net: ...")`를
   전부 제거. 매 프레임 도는 핫패스(DetectPickups/TickRespawns)에도 있으니 콘솔 스팸 정리 겸.
   (기능 변경 아님 — 컴파일 체크 + 씬 스모크만 돌리면 됨.)
2. **Netcode Milestone 3(Unity Relay/Lobby)**: 직접 IP 대신 방 코드 발급/입장 — Unity Cloud
   프로젝트 이미 연동됨(`cloudProjectId` 확인됨). 온라인 넷코드의 마지막 남은 조각.
