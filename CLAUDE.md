...
## 다음 단계 후보 (19차 작업)
1. **사용자 재플레이 확인**: 18차에서 1.5배로 늘어난 트랙 길이(비키니시티 459m/아프리카TV
   666m/네더요새 285m)가 목표한 "아프리카TV 빠르게 3바퀴 = 약 1분 30초" 체감과 맞는지,
   네더요새가 이제 충분히 긴지 확인.
2. **팔레트 텍스처 1장 확정 + Kenney 팩에서 3개 스테이지 배경 소품 선정/임포트**(사용자가 이번
   순서로 지정한 다음 항목) — 가장 빠르게 "빈 배경" 문제 해결.
