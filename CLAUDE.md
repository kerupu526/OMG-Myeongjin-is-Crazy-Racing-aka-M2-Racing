...
## 진행 상황 (2026-07-10 기준, 10차 작업 완료 — 실제 플레이테스트 기반 수정)
...
- **10차 작업 완료(2026-07-10) — 트랙 벽/표면 자기교차 버그 재진단·해결, 벽 충돌 피드백 개선, 확대,
  ...
  - **비키니시티 정식 씬 분리 착수** (다음 단계 후보 3번 완료, 비키니시티 한정): `TestTrackBuilder.Build`가
    ...
    **고정(freeze)의 트레이드오프**: 이 씬은 저장 시점 스냅샷이라 이후 `TestTrackBuilder` 변경사항이
    ...
    `StageTestSelector` 그대로라 상수 변경이 항상 자동 반영됨. 아직 검증 순서(컴파일 체크 →
    `BuildBikiniCityScene` → `SceneSmokeTest` → PlayMode 전체)를 실행하지 못함(에디터가 열려있어 배치
    락 충돌) — **다음 세션에서 에디터를 닫은 뒤 반드시 순서대로 실행해서 확인할 것.**
...
## 다음 단계 후보 (11차 작업)
1. **10차 작업 검증 마무리**: 에디터 닫고 `check_build.ps1`(컴파일) →
   `check_build.ps1 -Method BuildCheck.BuildBikiniCityScene`(씬 생성) →
   `check_build.ps1 -Method BuildCheck.SceneSmokeTest`(씬에 GameManager 있는지) →
   `check_playtest.ps1`(회귀) 순서로 실행, 마지막으로 에디터에서 `Stage_BikiniCity.unity`를 Play로
   눈으로 확인.
