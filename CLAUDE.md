...
- 네트워킹: Netcode for GameObjects + Unity Relay + Unity Lobby (방 코드 생성/입장, 온라인 1v1)
  ...
  - 27차 작업(2026-07-12)에서 착수. Netcode for GameObjects는 Milestone 1(기본 연결 + 차량 위치
    동기화, 직접 IP 접속)까지 진행됨 — Relay/Lobby는 아직 연결 안 됨. 자세한 내용은 아래
    "진행 상황"의 27차 작업 항목 참고.
...
- **27차 작업 착수(2026-07-12) — Netcode for GameObjects Milestone 1: 기본 온라인 연결 +
  ...
  - **중요한 한계(다음 세션에서도 계속 적용됨)**: 이 환경은 실제 네트워크 피어 2개를 붙여서
    자동 검증하는 게 근본적으로 불가능함 — Windows-MCP 마우스/키보드 자동화가 이미 이전 세션에서
    실제 Unity 게임플레이 입력(새 Input System)에 안 먹힌다는 게 확인된 상태고, 이번엔 GUI 에디터
    실행 자체도 반복적으로 막힘. **실제 호스트+클라이언트 접속/차량 동기화 확인은 사용자가 직접
    두 개의 실행 인스턴스로 해야 함**:
    1. `M2/Build Network Prefabs` 메뉴 → `M2/Build Persisted Scene/Network Bootstrap
       (Milestone 1)` 메뉴 순서로 먼저 실행(또는 이미 커밋된 결과물 그대로 사용).
    2. 에디터에서 `Assets/Scenes/NetworkBootstrap.unity`를 열고 Play. "호스트로 시작" 클릭.
    3. 두 번째 인스턴스가 필요함 — **ParrelSync**(무료 오픈소스 Editor 확장, 프로젝트를 심볼릭
       링크로 복제해서 두 번째 에디터 창을 여는, 정확히 이런 멀티플레이어 테스트 상황을 위해
       만들어진 도구) 설치를 권장. 또는 File > Build Settings로 Development Build 하나를 뽑아서
       빌드된 실행 파일 + 에디터 Play를 각각 호스트/클라이언트로 써도 됨.
    4. 두 번째 인스턴스에서 IP 입력창에 `127.0.0.1` 그대로 두고 "접속" 클릭.
    5. 접속되면 양쪽 화면에 차량 2대가 좌우로 벌어져 스폰되고, 각자 자기 차만 조작 가능해야
       하며, 한쪽에서 몬 차가 다른 쪽 화면에서도 (약간의 지연은 있을 수 있지만) 따라 움직여야 함.
    이 확인이 끝나기 전까지는 이번 마일스톤이 "코드는 다 있고 자동 검증도 통과했지만 실제 동작은
    미확인" 상태임을 알고 있을 것.

### 미착수
- **우선순위 4 (Netcode) — Milestone 1까지만 진행**: 아이템 스폰/획득/사용(폭탄 등)의 서버-권위
  동기화, 레이스 상태(타이머/승패)의 서버-권위 동기화, Unity Relay(방 코드 발급/입장), Unity
  Lobby(대기방/방 목록) 전부 다음 라운드 범위. 로컬 핫싯(2P 스플릿)은 CLAUDE.md 기획 자체가
  "온라인 1v1"만 명시하고 있어서 범위 아님.
...
## 다음 단계 후보 (28차 작업)
1. **사용자 확인(최우선, 새로운 종류의 확인)**: 27차의 Netcode Milestone 1이 실제로 동작하는지
   — 위 "중요한 한계" 절의 단계별 안내대로 두 개의 실행 인스턴스(ParrelSync 권장)로 직접 접속해서
   차량 동기화를 확인. 이전까지의 "차를 몰아보고 버그를 알려달라"는 방식과 달리 이번엔 두 인스턴스가
   필요한 새로운 유형의 확인 작업.
2. 위 확인 결과에 따라 Netcode Milestone 2(아이템/레이스 상태 서버-권위 동기화) 또는 Milestone 3
   (Relay/Lobby 연동)으로 진행할지, 아니면 25차에서 남겨둔 배경 소품/프리미티브 오브젝트 육안
   확인이나 미확정 디자인 항목(아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP
   리스킨 여부, 아프리카TV/네더요새 게이지 밸런스 수치) 정리로 돌아갈지 결정.
...
Assets/
  ...
  Scenes/ (MainMenu, Lobby, Stage_BikiniCity, Stage_AfricaTV, Stage_NetherFortress, ResultScreen,
    NetworkBootstrap — Netcode Milestone 1용, 27차 작업)
  Art/{Sprites, UI}/
```