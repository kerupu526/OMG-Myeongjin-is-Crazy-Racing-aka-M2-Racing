...
## 다음 단계 후보 (15차 작업)
사용자가 14차 수정사항을 실제로 플레이해보고 확인 완료함("오 됐다") — 방송사고존 크기/아이템
판정/트랙 길이 전부 만족스러운 상태.
1. **상태이상(디버프) HUD 표시(사용자 제안, 우선)**: 화면 우측 상단에 현재 걸려있는 상태이상
   아이콘/텍스트 표시 — 조향 반전(`BroadcastAccidentZone`), 기절/히트스턴
   (`VehicleController.ApplyHitStun`), 멘탈 붕괴 조작불가(`AfricaTvMentalGauge` lockout),
   화상 임박 경고(`NetherFortressTemperatureGauge` warning) 등 이미 존재하는 상태 이벤트들을
   한 곳에 모아서 보여주는 공용 UI 컴포넌트 필요. 현재 각 스테이지 UI(`BikiniCityStageUI`
   등)가 게이지별로 따로따로 표시 중이라, "지금 내 차에 뭐가 걸려있는지" 한눈에 안 보임 —
   `VehicleController`가 이미 관련 이벤트(`OnHitByAttackItem`, 스티어링 반전 등)를 노출하고
   있으니 새 상태이상 목록 UI가 이걸 구독하기만 하면 될 듯.
