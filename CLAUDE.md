...
- **아프리카TV(멘탈 게이지) — 구현 완료, `TestTrackBuilder` 배선은 아직 안 함**
  ...
  - **CLAUDE.md가 멘탈 게이지의 초당 변화량/피격당 상승폭을 명시하지 않아 플레이스홀더 수치로 구현함
    (밸런스 확정 필요).**
- **네더요새(체온 게이지) — 구현 완료, `TestTrackBuilder` 배선은 아직 안 함**
...
### 미착수
- **우선순위 3 잔여**: 아프리카TV/네더요새 둘 다 로직은 구현됐지만 `TestTrackBuilder`에 배선(테스트
  트랙에서 실제로 켜서 플레이테스트 가능한 상태)은 아직 안 됨 — 3개 스테이지가 동시에 한 차량에
  붙는 게 말이 안 되므로, 스테이지별로 트랙을 분리해서 만들 시점(우선순위 3/5, 정식 씬 전환)에
  같이 정리하기로 함.
...
## 다음 단계 후보 (7차 작업)
1. 정식 스테이지별 씬/트랙 분리 착수 — 지금까지는 `TestTrackBuilder` 하나로 비키니시티 게이지만
   테스트해왔는데, 아프리카TV/네더요새까지 실제로 플레이해보려면 스테이지별 트랙(또는 최소한
   `TestTrackBuilder`에 스테이지 선택 파라미터)이 필요함.
