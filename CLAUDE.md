...
## 다음 단계 후보 (26차 작업)
1. **사용자 확인(최우선)**: 25차에서 합성한 프리미티브 오브젝트(용암존 바위, 오아시스 갈대,
   방송 카메라 삼각대, 파이어볼 스파이크, 경고 표지판)의 크기/배치/색감이 실제 플레이 화면에서
   어색하지 않은지 육안 확인 — 자동화 환경에서는 차를 몰아볼 수 없어서 확인 못 함.
2. 진행하면서 미확정 항목(아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP 리스킨 여부,
   아프리카TV/네더요새 게이지 밸런스 수치)도 같이 정리.
3. **우선순위 4 (Netcode)**: 아직 전혀 착수 안 함. 로컬 코어 루프(우선순위 1~3)는 이제 3개 스테이지
