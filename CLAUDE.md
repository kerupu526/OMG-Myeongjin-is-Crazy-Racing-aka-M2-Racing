...
## 다음 단계 후보 (22차 작업)
1. **사용자 재플레이 확인(최우선)**: 21차의 역주행 방지(15m 후진 후 진짜로 막힘 + 배너가
   막혀있는 동안 계속 표시)가 이제 실제로 발동하고 자연스럽게 느껴지는지 확인.
   `maxReverseDistance`(15m)가 너무 짧아서 정상적인 벽 탈출/재정렬까지 막아버리지는 않는지도
   같이 체크 필요.
