...
- **우선순위 5**: 아이템 최종형 로스터, 정식 스테이지 3종 아트/씬, 나머지 승리조건(별점 내기) 확장,
  ...
  플레이스홀더).
- **아트 에셋 자체**: 트랙 표면/그라운드 팔레트는 19차, 배경 소품 종류/밀도는 24차에서 확충.
  Blockbench로 스테이지 고유 오브젝트 제작(용암존/방송세트/지형지물 등 플레이스홀더 큐브
  교체)은 아직 안 함.

## 다음 단계 후보 (25차 작업)
1. **사용자 확인(최우선)**: 24차에서 늘린 배경 소품(특히 비키니시티 야자수/바위의 플랫컬러
   폴백)이 실제 플레이 화면에서 어색하지 않은지 육안 확인 — 자동화 환경에서는 차를 몰아볼 수
   없어서 확인 못 함.
2. Blockbench로 스테이지별 고유 오브젝트(비키니시티 지형지물, 네더 용암존, 아프리카TV 방송 세트 소품 등)
   제작 → 현재 플레이스홀더 프리미티브(큐브 등) 교체.
3. 진행하면서 미확정 항목(아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP 리스킨 여부,
   아프리카TV/네더요새 게이지 밸런스 수치)도 같이 정리.
4. **우선순위 4 (Netcode)**: 아직 전혀 착수 안 함. 로컬 코어 루프(우선순위 1~3)는 이제 3개 스테이지
