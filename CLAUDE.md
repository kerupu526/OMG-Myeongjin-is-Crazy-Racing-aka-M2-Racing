...
## 진행 상황 (2026-07-11 기준, 12차 작업 완료 — 아프리카TV/네더요새 정식 씬 분리)
...
## 다음 단계 후보 (13차 작업)
1. **에디터에서 직접 플레이 확인 필요(우선)**: 12차 작업은 헤드리스 환경이라 아프리카TV/네더요새의
   새 트랙을 실제로 달려보지 못함 — `Stage_AfricaTV.unity`/`Stage_NetherFortress.unity`를 열어 직접
   플레이하며 드리프트/벽 충돌/게이지가 새 코너 레이아웃에서도 잘 맞는지 확인할 것. 특히 아프리카TV의
   12.1m로 좁아지는 한 지점(에시스 근처)이 실제로 타보면 너무 좁게 느껴지는지 체크.
2. **드리프트/트랙 밸런스 튜닝**: `driftTurnMultiplier`/`driftSlipRecoverySpeed`/
   `maxDriftBoostSpeed`/`driftBoostChargeTime`/`driftBoostDuration`(전부 플레이스홀더 수치)를 이제
   트랙이 3종 다 다르므로 각 트랙에서 실제로 여러 판 플레이테스트하면서 확정. 컨트롤 포인트 좌표를
   추가로 미세조정할 경우 반드시 Node.js 곡률/자기교차 검증부터 먼저 하고 코드에 반영할 것(이번에도
   네더요새 초안에서 오버슈트로 자기교차가 난 전례 있음).
