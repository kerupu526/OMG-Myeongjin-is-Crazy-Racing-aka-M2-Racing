...
## 진행 상황 (2026-07-10 기준, 11차 작업 완료 — 실제 플레이테스트 기반 수정)
...
## 다음 단계 후보 (12차 작업)
1. **드리프트/새 트랙 밸런스 튜닝**: `driftTurnMultiplier`/`driftSlipRecoverySpeed`/
   `maxDriftBoostSpeed`/`driftBoostChargeTime`/`driftBoostDuration`(전부 플레이스홀더 수치)를 실제
   여러 판 플레이테스트하면서 확정. 새 코너 레이아웃(시케인/헤어핀/스위퍼)도 체감 난이도 봐가며
   컨트롤 포인트 좌표 미세조정 가능성 있음 — 조정할 경우 반드시 Node.js 곡률/자기교차 검증부터
   먼저 하고 코드에 반영할 것(이번에 오버슈트로 곡률 반경 1m까지 떨어진 전례 있음).
2. 팔레트 텍스처 1장 확정 + Kenney 팩에서 3개 스테이지 배경 소품 선정/임포트(가장 빠르게 "빈 배경"
   문제 해결).
3. Blockbench로 스테이지별 고유 오브젝트(비키니시티 지형지물, 네더 용암존, 아프리카TV 방송 세트 소품 등)
   제작 → 현재 플레이스홀더 프리미티브(큐브 등) 교체.
4. 아프리카TV/네더요새도 비키니시티처럼 정식 씬으로 분리할지 결정 (`Assets/Scenes/Stage_AfricaTV.unity`,
   `Stage_NetherFortress.unity`) — 아트가 어느 정도 갖춰진 뒤로 미룰지, 지금 구조(뼈대)만 먼저 뗄지.
   분리한다면 이번 비키니시티 트랙처럼 각자 손으로 설계한 코너 레이아웃을 새로 만들지, 아니면 계속
   `TestTrackBuilder`의 공용 트랙(현재는 비키니시티용 레이아웃)을 재사용할지도 같이 결정 필요.
5. 진행하면서 미확정 항목(아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP 리스킨 여부,
   아프리카TV/네더요새 게이지 밸런스 수치)도 같이 정리.
6. **우선순위 4 (Netcode)**: 아직 전혀 착수 안 함. 로컬 코어 루프(우선순위 1~3)는 실질적으로 안정화됐고
   드리프트까지 들어왔으니, 아트 파이핑을 더 미루고 Netcode/Relay/Lobby로 넘어갈지 이번 참에 결정할 것.
