...
- 기존 테스트: `VehicleControllerTests`, `LapTrackerTests`, `GameManagerRaceFlowTests`,
  `BikiniCityOxygenGaugeTests`, `ScreenshotSmokeTest`(시각 확인용, 렌더링 불가 환경에서는
  `Inconclusive` 반환).
...
- **주의**: 디스플레이 없는 헤드리스 환경에서는 기본값(`-nographics`)으로 실행해야 함 — 아니면 그래픽
  ...
  보고됨(정상 — 실제 디스플레이 있는 머신에서 `-NoGraphics:$false`로 재실행하면 이미지 확인 가능).
- **주의**: 배치모드 Unity 실행이 완료 신호(로그 마지막 줄 출력)를 내보내기 전에 프로세스가 몇 분간
  조용히 계속 실행 중일 수 있음(특히 asmdef 추가 등으로 전체 재임포트가 걸릴 때). 5분 내로 끝나지
  않는다고 바로 재실행하지 말고, `playtest_results.xml` 생성 여부를 폴링해서 확인할 것 — 너무 빨리
  재실행하면 "다른 Unity 에디터에서 이 프로젝트가 열려 있습니다" 락 충돌이 남.
...
- 키보드 입력 시뮬레이션은 `InputTestFixture` 상속 + `InputSystem.AddDevice<Keyboard>()` 사용.
  단, 테스트 도중(특히 여러 프레임 대기 후) `Press()`/`Release()`로 캡처해둔 컨트롤 참조를 다시
  쓰면 `ArgumentNullException: does not have an associated state` 가 날 수 있음 — `Keyboard.current`로
  그때그때 새로 가져오거나, 굳이 필요 없으면 `Release()` 호출 자체를 생략할 것(`TearDown()`이 알아서
  정리함).
