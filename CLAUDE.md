...
## 다음 단계 후보 (30차 작업)
0. **Milestone 2a 라이브 확인 마저(최우선)**: 29차 후속수정 3번(접속 후 UI 숨김)이 실제로 되는지 +
   2명 접속 시 카운트다운/바퀴 카운트/타이머/승패 복제가 맞는지 확인. Console에 `M2Net:` 진단 로그가
   찍히니, 문제 시 그걸로 어디서 막히는지 알 수 있음(등록→스폰 순서). 이 진단 로그(`Debug.Log`)는
   확인 끝나면 지워도 됨.
1. **Milestone 2a 라이브 확인**: 사용자가 ParrelSync 호스트+클론으로 `NetworkRace.unity`를
