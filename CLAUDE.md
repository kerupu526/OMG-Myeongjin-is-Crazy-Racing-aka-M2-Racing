...
- **우선순위 5**: 아이템 최종형 로스터, 정식 스테이지 3종 아트/씬, 나머지 승리조건(별점 내기) 확장,
  ...
  플레이스홀더).
- **아트 에셋 자체**: 트랙 표면/그라운드 팔레트는 19차에서 확정(UI 디자인 시안 스타일). 실제
  Kenney 배경 소품 선정/임포트, Blockbench 모델 제작은 아직 안 함.

## 다음 단계 후보 (23차 작업)
1. **사용자 재플레이 확인(최우선)**: 22차에서 재설계한 역주행 방지가 이제 "후진"과 "차 돌려서
   반대로 전진" 두 경우 모두 실제로 막고, 배너도 잘 뜨는지 확인. `maxWrongWayDistance`(15m)가
   너무 짧아서 정상적인 벽 탈출/재정렬/코너 재진입까지 막아버리지는 않는지도 같이 체크 필요
   (특히 헤어핀 코너 근처에서 체크포인트 방향과 실제 도로 곡선이 크게 어긋나는 구간이 있는지
   — 직선 거리 기준 판정이라 급커브에서 오탐할 여지가 이론상 있음).
2. **Kenney 팩에서 3개 스테이지 배경 소품 추가 선정/임포트** — 현재 스테이지당 3종/10개뿐인
   배경 밀도를 늘려서 "빈 배경" 문제를 마저 해결. 새 팔레트(다크 네이비 트랙)에 맞춰 소품
   자체의 톤도 같이 고려.
3. Blockbench로 스테이지별 고유 오브젝트(비키니시티 지형지물, 네더 용암존, 아프리카TV 방송 세트 소품 등)
   제작 → 현재 플레이스홀더 프리미티브(큐브 등) 교체.
4. 진행하면서 미확정 항목(아이템 슬롯 교체 규칙, 막대형 수류탄 트리거 거리, 비키니시티 IP 리스킨 여부,
   아프리카TV/네더요새 게이지 밸런스 수치)도 같이 정리.
5. **우선순위 4 (Netcode)**: 아직 전혀 착수 안 함. 로컬 코어 루프(우선순위 1~3)는 이제 3개 스테이지
